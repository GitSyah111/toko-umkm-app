        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <h3 class="text-lg font-semibold mb-4">Edit Ulasan Produk: {{ $review->produk->nama_produk }}</h3>
                    ...
                    <form action="{{ route('buyer.reviews.update', $review->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="rating" name="rating" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="5" {{ (old('rating', $review->rating) == '5') ? 'selected' : '' }}>5 - Sangat Bagus</option>
                                <option value="4" {{ (old('rating', $review->rating) == '4') ? 'selected' : '' }}>4 - Bagus</option>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="komentar" name="komentar" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('komentar', $review->komentar) }}</textarea>
                            @error('komentar')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('buyer.reviews.index') }}" class="text-gray-500 hover:text-gray-700 mr-4">Batal</a>
